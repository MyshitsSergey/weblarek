...
*   **Конструктор класса**:
    *   `constructor(baseUrl: string, options?: RequestInit)` — принимает базовый адрес API и дополнительные опции для fetch.
    *   Внутри создаёт экземпляр `Api` и сохраняет его в приватное поле.

*   **Поля класса**:
    *   `private api: Api` — экземпляр базового класса для выполнения запросов.

*   **Методы класса**:
    *   `getProducts(): Promise<IProduct[]>` — делает GET-запрос на эндпоинт `/product/`. Возвращает промис с массивом товаров.
